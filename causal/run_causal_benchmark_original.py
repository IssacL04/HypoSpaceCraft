...
class CausalBenchmarkEnhanced:
    ...
    def create_prompt_with_cot(self, observations: List[Dict], prior_hypotheses: List[CausalGraph]) -> str:
        ...
        Prior predictions (try to find a different valid graph if possible):
        {prior_block}
        
        Task:
        Think step-by-step to find a directed acyclic graph (DAG) that explains all observations.
        
        REASONING STEPS (be concise):
        1. For each observation, note which nodes are affected (value=1) when a node is perturbed
        2. Infer which edges must exist (perturbing X affects Y means X has a path to Y)
        3. Eliminate impossible edges (perturbing X doesn't affect Y means no path from X to Y)
        4. If multiple valid graphs exist, prefer one with different edges from prior predictions
        ...
        Rules:
        - Use only the listed nodes
        - No self-loops, no cycles
        - The very last line MUST start with "FINAL ANSWER: Graph:"
        - Be concise in reasoning (2-4 sentences total)
        - MOST IMPORTANT: The graph must correctly explain ALL observations
        """
        return dedent(prompt).strip()
    ...
    def parse_llm_response(self, response: str) -> Optional[CausalGraph]:
        ...
        # For CoT responses, prioritize "FINAL ANSWER:" marker
        ...
        if final_answer_match:
            ...
            # Look for the LAST occurrence of "Graph:" (for CoT responses)
            all_graph_matches = list(re.finditer(r'(?i)\bgraph\s*:\s*(.+)$', s, flags=re.MULTILINE))
            if all_graph_matches:
                # Use the last match (most likely the final answer in CoT)
    ...
    def evaluate_single_observation_set(
        ...
        for i in range(n_queries):
            # Use CoT prompt if enabled, otherwise use standard prompt
            if self.use_cot:
                prompt = self.create_prompt_with_cot(observations, all_hypotheses)
            else:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if verbose:
                        print(f"  鈿? Exception on query {i + 1}: {str(e)[:100]}")
    ...
    def run_benchmark(
        ...
        for metric_name, metric_key in [('Parse Success Rate', 'parse_success_rate'),
            ...
            print(f"\n{metric_name}:")
            print(f"  Mean 卤 Std: {stats_dict['mean']:.3f} 卤 {stats_dict['std']:.3f}")
